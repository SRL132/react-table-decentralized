...
export default function FilterGroup({
  ...
  const getFilterOptions = (optionKey: string, optionValue: string) => {
    const index = state.filterOptions.findIndex(
      (el: filterOptions) => el.filterBy === searchBy
    );
  ...
  const handleCheckboxChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked === true) {
      dispatch({
        ...
        payload: [
          ...state.filterOptions,
          { filterBy: event.target.name, filterParam: "true" },
        ],
      });
    } else {
      dispatch({
        type: updateFilters,
        payload: state.filterOptions.filter(
          (options: filterOptions) => options?.filterBy !== event.target.name
        ),
      });
    }
  };
    ...
    <div>
      ...
      <div className="d-flex justify-content-center">
        {Object.keys(fields).map(
          (fieldString) =>
            fields[fieldString].type === "boolean" && (
              <div key={fieldString} className="d-flex align-items-center">
                <input
                  className="form-check-input"
                  name={fields[fieldString].name}
                  onChange={handleCheckboxChange}
                  type="checkbox"
                />
                <label
                  className="form-check-label"
                  htmlFor={fields[fieldString].name}
                >
        ...
        <div className="d-flex align-items-center">
          ...
          >
            <option value="q" data-tokens="all">
              {"All"}
            </option>
            {Object.keys(fields).map(
              (fieldString) =>
                fields[fieldString].type !== "boolean" &&
                fields[fieldString].type !== "keyValueArray" && (
                  <option
                    key={fieldString}
                    value={fields[fieldString].name}
                    data-tokens={fields[fieldString].label}
                  >
                    {fields[fieldString].label}
                  </option>
                )
